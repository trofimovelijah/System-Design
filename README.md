...
4. Сервис рекомендаций Recommendation service требует быстрый доступ к данным, для чего можно использовать Redis.
5. Sorting service использует для сортировки и ранжирования в режиме реального времени опять же Redis.
6. Push Active service аналогичен ранее рассмотренному Notification service, поэтому также применим колоночную БД Cassandra для хранения данных о различных типах уведомлений.
