...
4. Поскольку необходима обработка данных в режиме реального времени, то целесообразно большиие данные в Hadoop только хранить, а непосредственно обработку применять не через хадуповский MapReduce, а через Apache Spark. Это ускорит вычисления для используемых трёх ML-сервисов.
...
## Компонентный дизайн
1. Сбор данных о событиях через Apache Kafka. В системе используется отдельный экземпляр брокера сообщений, дабы не смешивать задачи по ML с функционированием остальной системы (из первых двух уровней), для которой развёрнут отдельная шина.
2. Под вопросом использования в ETL service конвейеров для обработки и хранения данных, например, Greenplum, поскольку уже имеется связка HDFS+Kafka.
3. Для сервиса Push Active service схема фактически дублирует аналогичную инфраструктуру сервиса Notification service.
4. Сервис аналитических отчётов должен отображать данные на выстроенной пользовательской стороне:
...
5. Apache Spark получает данные их брокера сообщений через Streaming, кладёт их в Hadoop, а затем уже задания запускаются как отдельные job'ы, что позволяет масштабировать схему вычислений.
6. Данные, полученные из Spark Jobs предоставляются сервису рекомендаций, который "быстрые" результаты хранит в Redis.
