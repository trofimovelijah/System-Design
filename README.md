...
3. После локализации местонахождения из Location service данные через сборщик местоположений (веб-сервер + колоночная БД) попадает в геопространственную БД PostGIS
4. Взаимодействие основных сервисов через Kafka

![изображение](https://github.com/trofimovelijah/System-Design/assets/15788014/6a580adb-4d07-4c3c-abf9-268e13dbc806)

## Масштабирование системы и повышение надёжности
5.1. Масштабирование достигается использованием балансировщика нагрузки между пользовательской страницей Main Page на фронтенде и сервисами автозаполнения и геопоиска, а также между этими же сервисами сервисами логирования и аналитики. Использование балансировщика нагрузки в связке с иными сервисами в рамках данного дизайна выглядит лишним, поскольку оставшиеся сервисы взаимодействуют непосредственно с БД.
5.2. Разумно использовать LB для сервиса POI service, хотя тема сбора данных в данной задаче не поднимается.
5.3. У балансировщиков используем резервные сервера для повышения избыточности.
5.4. Каждый сервис уместно реализовать из нескольких инстансов, чтобы в случае недоступности одного из них выполнение функций возлегло на свободный инстанс.
5.5. POI DB нужно реплицировать, также как и в случае с документной БД сервиса логирования

6. Логичное рассмотрение вопроса повышения отзывчивости
6.1. Пользовательский кэш используем для решения сценария отображения наиболее частых запросов пользователя.
6.2. Для взаимодействия между клиентом и сервисом используем веб-сокеты. Сюда же уместно использовать веб-сокет для взаимодействия с внешним API.
6.3. При осуществлении геопоиска данные берутся из кэша, созданного в обход из POI DB.
6.4. Сервис автозаполнения вначале помещает данные реверсивно в кэш Food & Rest быстрого хранилища Redis (см. выше), а затем уже в колоночную БД.
6.5. Сервис Indexing AutoComplete service индексирует данные, взятые из документной базы сервиса логирования и использует их для автозаполнения вводимых в поисковом запросе данных.
