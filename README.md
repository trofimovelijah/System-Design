...
8.
9. Пользовательский кэш используем для решения сценария отображения наиболее частых запросов пользователя.
10. Для взаимодействия между клиентом и сервисом используем веб-сокеты. Сюда же уместно использовать веб-сокет для взаимодействия с внешним API.
11. При осуществлении геопоиска данные берутся из кэша, созданного в обход из POI DB.
12. Сервис автозаполнения вначале помещает данные реверсивно в кэш Food & Rest быстрого хранилища Redis (см. выше), а затем уже в колоночную БД.
